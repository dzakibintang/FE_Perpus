...
export default function BukuPage() {
    ...
    if (editing) {
      ...
      try {
        const res = await fetch(
          (`http://localhost:8000/api/updatebuku/${editId}`),
          {
            method: "POST", // atau 'PUT' jika backend mendukungnya
  ...
  const handleDelete = async (id_buku) => {
    ...
    try {
      const res = await fetch(
      (`http://localhost:8000/api/deletebuku/${id_buku}`),
        {
          method: "DELETE",
        }
      );
    ...
    <Layout>
      <div className="container">
        ...
        <form onSubmit={handleSubmit} className="mb-4">
          ...
          <div className="mb-3">
            <label htmlFor="gambar_buku" className="form-label">
              Gambar Buku
            </label>
            <input
              type="file"
            ...
            {buku.length > 0 ? (
              buku.map((item) => (
                <tr key={item.id_buku}>
                  ...
                  <td>
                    {item.gambar_buku ? (
                      <img src={`http://localhost:8000/storage/${item.gambar_buku}`} alt={item.judul_buku} style={{ width: "80px", height: "auto" }} />
                    ) : (
                      "No Image"
                    )}
                  </td>
        ...
        {detail && (
          <div className="card mt-4">
            ...
            <div className="card-body">
              <p>
                <strong>Judul:</strong> {detail.judul_buku}
              </p>
              <p>
                <strong>Penulis:</strong> {detail.penulis}
              </p>
              <p>
                <strong>Penerbit:</strong> {detail.penerbit}
              </p>
              <p>
                <strong>Kategori:</strong> {detail.kategori}
              </p>
              {detail.gambar_buku && (
                <img
